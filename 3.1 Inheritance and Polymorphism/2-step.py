...
Метод dist_count(start, finish) будет статическим (@staticmethod). 
    ...
    Внутри метода создайте переменную dist = ((finish.x - start.x) ** 2 + (finish.y - start.y) ** 2) ** 0.5 
    - формула подсчитывает расстояние между точками. В итоге, метод выводит на экран округлённый результат round(dist) с помощью print.
