...
Маша, Вика и Лена, ещё только учатся считать, поэтому могут ошибиться. 
    Они взяли одинаковый товар на 1000 и подошли на кассу расплатиться. 
    Каждый из них достал денежку (number) и посчитал как мог, проверьте, достаточно ли они положили денег на кассе. 
    И не переживайте, денежки у них есть, просто могут ошибиться в подсчёте. 
Задание:
Часть кода уже написана.
Добавьте недостающий код, только в методе __new__.
В методе __new__ сделайте проверку, если number меньше 1000, то экземпляр не создаётся. 
    Если больше или равно, то экземпляр создастся. 
